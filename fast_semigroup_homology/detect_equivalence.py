...
def attempt_detect_equivalence(op1, op2, f, limit=None) -> bool:
    """
    ...
    return (
        ...
        and (
            slice_resolution(
                ...
                op2,
                f,
            ).has_trivial_homology(limit=limit)
            or
            slice_resolution(
                list(map(list, zip(*op1))), # reverse the operation
                list(map(list, zip(*op2))), # reverse the operation
                f,
            ).has_trivial_homology(limit=limit)
        )
    )

def equivalence_to_quotient(op, limit=None):
    """
    If this monoid has a quotient monoid such that
    Quillen's theorem A applies to the quotient map,
    then return the quotient operation and the quotient map.
    """
    for op2, f in proper_nontrivial_quotient_semigroups(op):
        if attempt_detect_equivalence(op, op2, f, limit=limit):
            return op2, f
    return None

def proper_nontrivial_quotient_semigroups(op):
    """
    Produce pairs (op2, f) where op2 is a multiplication table for a
    quotient semigroup of op and where f is the quotient mapping.
    """
    n = len(op)
    for cong in congruences(op):
        ...
        assert {x for comp in components for x in comp} == set(range(n))
...
def congruences(op):
    """
    Using Berhard Ganter's NextClosure algorithm
    (Alg 1.7 from https://doi.org/10.1007/978-3-642-11928-6_22),
    search for all congruences on a monoid.

    A congruence is an equivalence relation on pairs
    such that if x ~ y then uxv ~ uyv. This function
    yields sets of only those pairs (i, j) of ints in
    the relation with i <= j.
    """
    n = len(op)
    rn = range(n)
    ...
    while True:
        ...
        for p in reversed(potential_pairs):
            ...
            for q in new_implied_congruences(op, A, p):
                ...
                # Do this here to try to avoid so much copying
                # in short-circuiting cases.
                Ap.update(A)
                history.append((p, Ap))
                break # back to yield
        else:
...
def new_implied_congruences(op, pairs, new_pair):
    """
    Given a congruence on a semigroup and a new pair of elements,
    yield the pairs in the congruence generated by the existing
    congruence and the new pair, excluding those pairs
    already in the existing congruence.
    """
    def equiv_closure():
        progress = False
        for comp in get_components(len(op), pairs):
    ...
    yield new_pair
    temp = {new_pair} # delay copying the whole set of pairs
    for pair in whiskered_congruences(op, new_pair):
        if pair not in pairs and pair not in temp:
            yield pair
            temp.add(pair)
    pairs = pairs | temp
    yield from equiv_closure()
    while True:
        if not (yield from mul_closure()):
            return
        if not (yield from equiv_closure()):
            return
...
def whiskered_congruences(op, pair):
    ...
    for x in range(len(op)):
        op_xa = op[op[x][a]]
        op_xb = op[op[x][b]]
        for xay_xby in zip(op_xa, op_xb):
            yield tuple(sorted(xay_xby))

def get_components(n, pairs):
    """depth-first search through a graph to list the connected components"""
    adjacencies = [set() for _ in range(n)]
    for x, y in pairs:
        adjacencies[x].add(y)
        adjacencies[y].add(x)
